...
def fetch_ev_data():
    """從 CEM 網站爬取實時 EV 充電位數據（使用 Playwright）"""
        ...
        async with async_playwright() as p:
            ...
            result = await page.evaluate('''
                data.list.map(s => {
                    ...
                    return { name: name, available: availablePiles };
                })
            ''')
            ...
            ev_data = {}
            for item in result:
                ev_data[item['name']] = item['available']
